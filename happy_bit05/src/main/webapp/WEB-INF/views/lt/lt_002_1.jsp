...
<%@ include file="../includes/middle.jsp"%>
	<form method="get" action="/lt/lt_003_1">
	<input type="hidden" name="lt_b_type" value="공유게시판">
		<div class="container">
		<div style="margin-top: 30px;">
			<h1>트레이닝 게시판<small> 공유 게시판</small></h1>
			</div>
		...
		<!-- 게시글 리스트 출력 테이블 -->
			<div class="table-responsive">
				<table class="table table-hover" style="margin-top: 30px;">
					...
					<tbody id="table_list">
						<c:forEach var="board" items="${list }">
							...
							<tr class="success" onclick="location.href='/lt/lt_004_1?index=${board.lt_index }'">
								<td id="index">${board.lt_index }</td>
								<td>${board.lt_tier }</td>
								<td>${board.lt_type }</td>
								<td class="title">${board.lt_title }</td>
								<td>${board.m_index }</td>
								<td>${board.lt_date }</td>
								<td>${board.lt_see }</td>
								<td>${board.lt_good }</td>
				...
				<div id="row">
					<div class="col-md-6">
						<a href="/lt/lt_003_1?lt_b_type=공유게시판"><button type="submit" class="btn btn-success">글작성</button></a>
					</div>
				<!-- 페이징  -->
					<div class="col-md-6">
						<ul class="pagination justify-content-end">
							<c:if test="${pageUtil.prev }">
								<li class="page-item"><a class="page-link" href="/lt/lt_002_1?page=${pageUtil.start-1}">Previous</a></li>
							</c:if>
							<c:forEach begin="${pageUtil.start }" end="${pageUtil.end }" var="pNum">
								<li class="page-item ${pNum==pageUtil.dto.page?'active':"" }"><a class="page-link" href="/lt/lt_002_1?page=${pNum }">${pNum }</a></li>
							</c:forEach>
							<c:if test="${pageUtil.next }">
								<li class="page-item"><a class="page-link" href="/lt/lt_002_1?page=${pageUtil.end+1 }">Next</a>
								</li>
							</c:if>
						</ul>
					</div>
