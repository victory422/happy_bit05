			...
			<div class="panel panel-default">
				<div class="panel-body">
					...
					<c:out value="${type1 eq '자전거'?'selected':''}"/>
								...
								placeholder="검색어 입력" onsubmit="page_put()" /> <a
								class="text-muted" aria-label="Search" name="search_button"
					...
					<div>
						<c:forEach items="${data}" var="data">
							...
							<table class="table table-bordered table-hover"
								style="margin-top: 30px;"
								onClick="location.href='/co/co_004_1?co_b_index=${data.co_b_index}'">
								<tr>
									<td rowspan="5"
										style="width: 200px; height: 125px; padding: 0;"><img
										id="thumbnail" src="data:image/jsp;base64, ${data.co_request}"
										style="width: 230px; height: 210px; padding: 0;"></td>
					...
					<ul class="pagination d-flex justify-content-center">
						<c:if test="${pageUtil.prev }">
							<li class="page-item"><a class="page-link"
								href="/co/co_003_1?page=${pageUtil.start-1}">Previous</a></li>
						</c:if>
						...
						<c:if test="${pageUtil.next }">
							<li class="page-item"><a class="page-link"
								href="/co/co_003_1?page=${pageUtil.end+1 }">Next</a></li>
						</c:if>
					</ul>
				</div>
			</div>
